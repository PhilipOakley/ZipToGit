...
# echo "$tempdir"
# Usage: $dirContaingZip> UZGC <YourZip> 
...
# echo "------- pipe diff diff, ls sort it"  
# git --work-tree="$tempsubdir" --git-dir="$gitdir/.git"\
  # diff  --no-renames --name-status --diff-filter=ACMRTB --src-prefix="$tempsubdir"\
   # --cached  HEAD \
 # | sed -e "s|^..|$tempsubdir|"\
...
git --work-tree="$tempsubdir" --git-dir="$gitdir/.git"\
  commit -a -m "$unzipfile" --allow-empty --author="$author1"  --date="$TimeDate"
